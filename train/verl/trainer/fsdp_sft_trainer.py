...
class FSDPSFTTrainer:
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh):
        ...
        # Check if the model is a Llama/Llama3 variant by inspecting the model type or path
        is_llama = (
            "llama" in local_model_path.lower()
            or getattr(self.tokenizer, "model_type", "").lower() in ("llama", "llama3")
            or "llama" in getattr(self.tokenizer, "name_or_path", "").lower()
        )
        if is_llama:
