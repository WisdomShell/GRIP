...
def run_test(in_outs, test=None, debug=False, timeout=15):
    ...
    maximum_memory_bytes = 20 * 1024 * 1024  # 20 MB in bytes
...
def process_ot(in_out, F):
    ret = []
    for item in in_out:
        if isinstance(item, list):
            item_new = json.dumps(item)
        else:
            if F:
                item_new = json.dumps(item)
            else:
...
def check_correctness(sample, generation, timeout, debug=True):
    ...
    if not result:
        ...
        result = [[False for i in range(len(in_outs["inputs"]))]]
        if debug:
