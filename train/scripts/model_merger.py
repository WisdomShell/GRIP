...
def save_tokenizer_and_extra_files(hf_model_path: str, target_dir: str):
    if not os.path.exists(hf_model_path):
        print(f"Warning: hf_model_path {hf_model_path} does not exist, skip copying tokenizer.")
        return

    print(f"Copying tokenizer and extra files from {hf_model_path} to {target_dir}")
    ...
    for item in items_to_copy:
        src = os.path.join(hf_model_path, item)
    ...
    try:
        ...
        print("Successfully saved tokenizer using AutoTokenizer.save_pretrained()")
    except Exception as e:
        print(f"Warning: Failed to save tokenizer with AutoTokenizer: {e}")
...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    save_tokenizer_and_extra_files(args.hf_model_path, hf_path)
